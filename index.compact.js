/* GENERATED_FROM=index.js SOURCE_SHA256=89835c5e448cf1212af0806b411284f8d901cec8d24022396bb37b84e39f20f4 FORMAT=READABLE_COMPACT PRINT_WIDTH=240 DO_NOT_EDIT */
'use strict';
...
const buildScaledRewards = ({ id, metric, targets, xpBase, xpMultiplier, channel = 'listening' }) =>
  targets.map((target, index) => ({ id: `${id}_${index + 1}`, metric, channel, target, amount: Math.floor(xpBase * Math.pow(xpMultiplier, index)), validatorVersion: 1 }));
...
async function actionSocialSessionIssue(event, body) {
  ...
  await kvPut({
    ...
    type: 'profile',
    owner: identity.friendId,
    data: { ...oldProfile, friendId: identity.friendId, displayName: identity.displayName || oldProfile.displayName || 'Слушатель', avatarUrl: identity.avatarUrl || oldProfile.avatarUrl || '', updatedAt: issuedAt }
  });
  ...
  const loyalty = await applyLoyaltyActivity(identity.friendId, { activityId: `loyalty:auth:${sessionJti}`, kind: 'authorization', deviceId: body.deviceId }).catch(error => ({
    loyalty: null,
    loyaltyRewards: [],
    wallet: null,
...
async function actionHeartbeat(event, body) {
  ...
  await kvPut({
    ...
    owner: playerId,
    expiresAt: until,
    data: { playerId, deviceId, currentGameId, currentRoomId, onlineUntil: until, publicIpHash: publicIpHash(event), updatedAt: now() }
  });
...
async function actionFriendInviteAccept(event, body) {
  ...
  await kvPut({
    pk: `friend:${inv.fromPlayerId}:${playerId}`,
    type: 'friend',
    owner: inv.fromPlayerId,
    data: { ownerPlayerId: inv.fromPlayerId, friendPlayerId: playerId, createdAt: at, status: 'active', profile: { displayName: myProfile.displayName, avatarUrl: myProfile.avatarUrl } }
  });
  ...
  if (inv.fromProfile)
    await kvPut({
      pk: `profile:${inv.fromPlayerId}`,
      type: 'profile',
      owner: inv.fromPlayerId,
      data: { friendId: inv.fromPlayerId, displayName: safe(inv.fromProfile.displayName || 'Друг').slice(0, 80), avatarUrl: safe(inv.fromProfile.avatarUrl || '').slice(0, 400), updatedAt: at }
    }).catch(() => null);
...
async function actionSignalSend(event, body) {
  ...
  await kvPut({
    ...
    owner: toPeerId,
    expiresAt,
    data: { seq, status: 'pending', deliveredAt: 0, deliveryAttempts: 0, roomId, fromPlayerId: playerId, fromPeerId, toPeerId, type, data: payloadData, createdAt: now(), expiresAt }
  });
...
async function sendSystemWebPush({ toPlayerId, title, body, url = './', tag = 'vi3-notification', requireInteraction = false, kind = '', fromFriendId = '', gameId = '', msgId = '', callId = '' } = {}) {
  ...
  try {
    const res = await fetch(CFG.webPushFunctionUrl, {
      ...
      body: JSON.stringify({ action: 'send_to_player', playerId: toPlayerId, title, body, url, tag, requireInteraction, kind, fromFriendId, gameId, msgId, callId })
    });
...
async function actionPushSend(event, body) {
  ...
  if (kind === 'GAME_INVITE') {
    ...
    if (
      ...
      (join.data.invitedPlayerId && join.data.invitedPlayerId !== toFriendId)
    ) {
...
function normalizeCryptoPack(raw) {
  ...
  const envelopes = (Array.isArray(pack.envelopes) ? pack.envelopes : [])
    .slice(0, 30)
    .map(item => ({ ownerId: sanitizeId(item?.ownerId), deviceId: sanitizeId(item?.deviceId, 120), wrapIv: safe(item?.wrapIv), wrappedKey: safe(item?.wrappedKey) }));
  ...
  if (
    ...
    envelopes.some(item => !item.ownerId || !item.deviceId || item.wrapIv.length > 100 || item.wrappedKey.length > 300)
  ) {
...
async function putChatReceipt({ room, message, playerId, kind }) {
  ...
  const receipt = {
    ...
    deliveredAt: old.deliveredAt || (kind === 'delivered' || kind === 'read' ? at : 0),
    readAt: old.readAt || (kind === 'read' ? at : 0),
    updatedBy: playerId,
    updatedAt: at
  };
...
async function actionChatSendV2(event, body) {
  ...
  await kvPut({
    ...
    owner: toFriendId,
    expiresAt,
    data: { pushId, msgId, fromFriendId: playerId, kind: 'CHAT_MESSAGE', cryptoVersion: 2, createdAt, expiresAt }
  });
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    input.forEach(raw => {
      ...
      const item = normalizeFavoriteItem(uid, {
        ...
        album: track.album,
        status,
        addedAt: Math.max(0, num(raw?.addedAt)) || at,
        updatedAt: at,
        inactiveAt: status === 'inactive' ? at : 0,
        deletedAt: status === 'deleted' ? at : 0,
...
const LOYALTY_DUE_BUCKET_MS = 5 * 60 * 1000;
...
function normalizeLoyaltyVacation(raw = {}) {
  return {
    ...
    resumeDeadlineAt: Math.max(0, Math.floor(num(raw.resumeDeadlineAt))),
...
function loyaltyVacationUsedMs(vacationRaw, at = now()) {
  ...
  let used = vacation.usage.reduce((sum, item) => {
    const from = Math.max(cutoff, item.startedAt);
    ...
    ...new Map(
      (Array.isArray(raw.trustFlags) ? raw.trustFlags : [])
        .map(flag => {
          ...
          return code
            ? [code, { code, severity: sanitizeId(flag?.severity || 'low', 20), firstSeenAt: Math.max(0, num(flag?.firstSeenAt)), lastSeenAt: Math.max(0, num(flag?.lastSeenAt)), count: Math.max(1, Math.floor(num(flag?.count, 1))) }]
            : null;
        })
        .filter(Boolean)
    ).values()
...
function publicLoyaltyState(raw = {}) {
  ...
  return {
    ...
    vacation: {
      ...
      resumeDeadlineAt: state.vacation.resumeDeadlineAt,
...
function listenContextVersion(body = {}) {
  ...
  return !!normalizeListenQuality(body.quality) &&
    ...
    Object.entries(source)
      .map(([deviceId, value]) => {
        ...
        return [
          id,
          {
            runMs: Math.max(0, Math.min(10800000, Math.floor(num(value?.runMs)))),
            lastTrackUid: sanitizeId(value?.lastTrackUid, 160),
            lastCompletedAt: Math.max(0, num(value?.lastCompletedAt)),
            updatedAt: Math.max(0, num(value?.updatedAt))
          }
        ];
      })
...
function publicConfirmedListeningStats(progressRaw) {
  ...
  return {
    ...
    legacyUnclassifiedMs: progress.legacyUnclassifiedMs,
    validPlays: progress.validPlays,
    fullPlays: progress.fullPlays,
    uniqueTracks: Object.keys(progress.uniqueTracks).length,
...
async function finalizeListenSession(session) {
  ...
  const full =
    ...
    data.lastPosition >= Math.max(0, data.duration - fullPositionToleranceSec) &&
  ...
  const completedReceipt = {
    ...
    rewardAmount: rewards.grants.reduce((sum, grant) => sum + grant.amount, 0),
    rewardIds: rewards.grants.map(grant => grant.achievementId)
  };
...
async function actionListenSessionHeartbeat(event, body) {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    const loyalty =
      observation.accepted === true && appliedTime.creditedMs > 0 && observation.session.acceptedHeartbeats === 1
        ? await applyLoyaltyActivity(playerId, { activityId: `loyalty:listen:${observation.session.sessionId}`, kind: 'listening', deviceId: observation.session.deviceId })
        : null;
...
function normalizeSleepTimer(raw = {}) {
  return {
    version: 1,
    timerId: sanitizeId(raw.timerId, 120),
    playerId: sanitizeId(raw.playerId, 96),
    deviceId: sanitizeId(raw.deviceId, 120),
    status: sanitizeId(raw.status || 'active', 30),
...
function normalizePwaInstallIntent(raw = {}) {
  return {
    version: 1,
    intentId: sanitizeId(raw.intentId, 120),
    playerId: sanitizeId(raw.playerId, 96),
    deviceId: sanitizeId(raw.deviceId, 120),
    platform: sanitizeId(raw.platform, 30),
    status: sanitizeId(raw.status || 'active', 30),
    challengeHash: safe(raw.challengeHash).slice(0, 64),
    createdAt: Math.max(0, num(raw.createdAt)),
    expiresAt: Math.max(0, num(raw.expiresAt)),
    completedAt: Math.max(0, num(raw.completedAt)),
    receiptId: sanitizeId(raw.receiptId, 160),
    updatedAt: Math.max(0, num(raw.updatedAt))
  };
...
async function actionAchievementRewardStatus(event, body) {
  ...
  return {
    ...
    catalog: {
      ...
      rewardItems: publicAchievementRewardItems({ playerId, progress, favoriteState, wallet: rewards.wallet })
    },
...
const SHARD_AVATAR_CATALOG = Object.freeze([
  Object.freeze({ id: 'avatar_dragon', avatar: '🐉', title: 'Дракон Витрины', price: 100 }),
  Object.freeze({ id: 'avatar_planet', avatar: '🪐', title: 'Осколок Вселенной', price: 500 }),
  Object.freeze({ id: 'avatar_crown', avatar: '👑', title: 'Корона Башни', price: 1000 })
]);
  ...
  const operations = Object.fromEntries(
    Object.entries(wallet.operations && typeof wallet.operations === 'object' ? wallet.operations : {})
      .map(([operationId, operation]) => [
        ...
        { kind: sanitizeId(operation?.kind, 30), matchId: sanitizeId(operation?.matchId, 120), amount: Math.max(0, Math.floor(num(operation?.amount))), at: num(operation?.at) }
      ])
...
function publicAchievementRewardItems({ playerId, progress, favoriteState, wallet }) {
  ...
  return achievementRewardCatalog(progress).map(reward => {
    ...
    return {
      ...
      target,
      current,
      cumulativeTarget: reward.target,
      progressOffset,
      progression: reward.progression || 'cumulative',
      amount: reward.amount,
      album: reward.album || '',
      validatorVersion: reward.validatorVersion,
      eligible: metricCurrent >= reward.target,
      awarded: grantIds.has(operationId),
      awardedAt: num(operation?.at),
...
async function actionWalletPurchaseAvatar(event, body) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    const next = {
      ...
      purchasedAvatarIds: [...wallet.purchasedAvatarIds, item.id],
      purchaseIds: [...wallet.purchaseIds, purchaseId].slice(-100),
      updatedAt: now()
    };
...
async function applyWalletEscrowOperation({ playerId, matchId, kind, amount, won = false }) {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    const next = {
      ...
      locked: nextLocked,
      locks,
      operations: trimWalletOperations({ ...(wallet.operations || {}), [operationId]: { kind, matchId, amount: stake, at } }),
      earned: nextEarned,
      spent: nextSpent,
      updatedAt: at
    };
...
const RANKED_TRANSITIONS = Object.freeze({
  pending: new Set(['forfeit_pending', 'settling', 'disputed', 'aborted']),
  forfeit_pending: new Set(['settling', 'disputed', 'aborted']),
  settling: new Set(['settled', 'forfeited', 'disputed', 'refunded']),
...
async function buildRankedSettlementPlan({ match, winnerId, loserId, outcome = 'completed', turns = 0 }) {
  ...
  return {
    ...
    economy: {
      ...
      status: safe(match.economy?.status || 'locking'),
      stakeEach: Math.max(0, Math.floor(num(match.economy?.stakeEach, RANKED_STAKE_AMOUNT))),
      escrow: Math.max(0, Math.floor(num(match.economy?.escrow, RANKED_STAKE_AMOUNT * 2)))
    },
    createdAt: now()
  };
}
...
async function settleRankedMatch(matchId) {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    const [winner, loser] = await Promise.all([
      applyRankedProfileSettlement({ playerId: settlement.winnerId, matchId, won: true, delta: num(settlement.winnerDelta) }),
      applyRankedProfileSettlement({ playerId: settlement.loserId, matchId, won: false, delta: num(settlement.loserDelta) })
    ]);
...
async function actionRankedMatchPrepare(event, body) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    const changed = await kvCompareAndPut({
      ...
      data: { ...currentRoom, rankedMatchId: candidate, rankedPreparedAt: preparedAt, updatedAt: preparedAt }
    });
...
async function actionRankedMatchAbort(event, body) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    const next = transitionRankedMatch(match, 'forfeit_pending', {
      reason,
      actorId: playerId,
      extra: { forfeit: { loserId: playerId, winnerId: otherRankedPlayer(match.participants, playerId), reason, explicit, requestedAt, deadlineAt } }
    });
...
async function actionWebPushSubscribe(event, body) {
  ...
  await kvPut({
    ...
    owner: playerId,
    expiresAt: 0,
    data: { playerId, endpointHash, subscription: sub, userAgent: safe(body.userAgent || '').slice(0, 220), createdAt: now(), updatedAt: now() }
  });
...
function loyaltyNotificationForDue(due, state) {
  ...
  return {
    ...
    body: `До окончания текущего дня Преданности остался один час. Проявите активность, чтобы сохранить серию и получить следующий бонус.`,
    ...
    if (action === 'ping') {
      return reply(event, 200, {
        ...
        featureRewards: {
          enabled: true,
          shadow: CFG.featureRewardsShadow,
          actions: ['music_feature_use', 'pwa_install_intent', 'pwa_launch_verify', 'sleep_timer_start', 'sleep_timer_complete', 'sleep_timer_cancel'],
          metrics: ['lyricsUsed', 'pwaInstalled', 'sleepTimerTriggers']
        },
