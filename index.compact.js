/* GENERATED_FROM=index.js SOURCE_SHA256=649327af05dcd88d44deef3b4b9c41f24cf1da3fb9508619569163b952d74f19 FORMAT=READABLE_COMPACT PRINT_WIDTH=320 BLANK_LINES=SAFE_REMOVE DO_NOT_EDIT */
'use strict';
...
function corsHeaders(event) {
  ...
  return { 'Access-Control-Allow-Origin': allow, 'Access-Control-Allow-Methods': 'POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type, Accept, Authorization, X-Vi3-Admin', 'Access-Control-Max-Age': '86400', 'Vary': 'Origin' };
}
...
function rowsOf(res) {
  ...
  const resultSet = sets.find(set => set?.rows?.length) || sets.find(set => set?.columns?.length) || null;
  const columns = (resultSet?.columns || []).map(column => safe(column.name || column));
  return (resultSet?.rows || []).map(row => {
    const items = row.items || row;
    if (!Array.isArray(items)) return row;
    return Object.fromEntries(items.map((item, index) => [columns[index] || `c${index}`, valueOf(item)]));
  });
}
...
function notificationTtl(kind) {
  ...
  if (kind === 'VOICE_CALL' || kind === 'GAME_INVITE') return 120;
  return 3600;
}
...
async function sendToSubscription(row, notification) {
  ...
  const topic = safe(notification.tag || '')
    ...
    .slice(0, 32);
  try {
    await webpush.sendNotification(sub, JSON.stringify(notification), { TTL: notificationTtl(notification.kind), urgency: notificationUrgency(notification.kind), topic: topic || undefined });
    return { ok: true };
  } catch (err) {
...
async function actionSendToPlayer(event, body) {
  const adminHeader = Object.entries(event.headers || {}).find(([key]) => String(key).toLowerCase() === 'x-vi3-admin')?.[1];
  if (!CFG.adminSecret || !timingSafeEqualText(adminHeader, CFG.adminSecret)) {
...
exports.handler = async event => {
  ...
  try {
    ...
    return reply(event, Number(err?.httpStatus || 500), { ok: false, error: safe(err?.message || 'server_error') });
  }
};
