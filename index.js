...
const corsHeaders = event => {
  ...
  return {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, Accept, X-Yandex-Auth, X-Vi3-Session',
    'Access-Control-Max-Age': '86400',
    Vary: 'Origin'
  };
};

const reply = (event, statusCode, body, id) => ({
  statusCode,
  headers: {
    ...corsHeaders(event),
...
async function readYandexIdentity(token) {
  const response = await request('GET', 'https://login.yandex.ru/info?format=json', {
  ...
  return {
    yandexId,
    ownerYandexIdHash: hash(`ya:${yandexId}`)
  };
}
...
async function authorizeRequest(event, body) {
  ...
  return {
    oauthToken,
    playerId: safeId(authorization.playerId, 96),
