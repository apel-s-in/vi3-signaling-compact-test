/* GENERATED_FROM=input.js SOURCE_SHA256=971a80a7d528f784eb20336fedfc42cc9c32f0d5eaba45c158212962bc7d6879 FORMAT=READABLE_COMPACT PRINT_WIDTH=320 BLANK_LINES=SAFE_REMOVE DO_NOT_EDIT */
const STORAGE_KEY = 'albumCarouselCenter:v1';
const SETTLE_MS = 520;
const SWIPE_DISTANCE = 34;
const FLICK_VELOCITY = 0.22;
const SLOT_CLASSES = ['album-carousel-slot-m3', 'album-carousel-slot-m2', 'album-carousel-slot-m1', 'album-carousel-slot-0', 'album-carousel-slot-p1', 'album-carousel-slot-p2', 'album-carousel-slot-p3', 'album-carousel-hidden'];
...
export const mountAlbumCarousel = ({ root, itemSelector = '.album-icon', onSettled } = {}) => {
  if (!root) return null;
  const sourceItems = [...root.querySelectorAll(itemSelector)].filter(item => !String(item.dataset.album || '').startsWith('__'));
  ...
  const render = () => {
    ...
    root.setAttribute('aria-label', `Альбомы. В центре: ${current?.title || getCurrentKey()}`);
  };
  ...
  const setCurrent = (keyOrIndex, { animate = true, notify = false, persist = true, reason = 'programmatic' } = {}) => {
    const index = typeof keyOrIndex === 'number' ? mod(keyOrIndex, items.length) : items.findIndex(item => item.dataset.album === String(keyOrIndex || ''));
  ...
  const step = (direction, reason = 'step') => setCurrent(center + direction, { animate: true, notify: true, persist: true, reason });
  const onClickCapture = event => {
    ...
    event.preventDefault();
    event.stopImmediatePropagation();
    setCurrent(index, { animate: true, notify: true, persist: true, reason: 'item_click' });
  ...
  const onPointerMove = event => {
    ...
    lastAt = now;
    dragX = dx;
    root.style.setProperty('--album-carousel-drag-x', `${Math.max(-72, Math.min(72, dx * 0.42))}px`);
  };
  const finishPointer = event => {
    ...
    const shouldMove = horizontalDrag && (Math.abs(dragX) >= SWIPE_DISTANCE || Math.abs(velocityX) >= FLICK_VELOCITY);
    if (shouldMove) {
  ...
  const onKeyDown = event => {
    if (event.key === 'ArrowLeft') {
      ...
      event.preventDefault();
      setCurrent(0, { animate: true, notify: true, persist: true, reason: 'keyboard_home' });
  ...
  setCurrent(savedKey || 0, { animate: false, notify: false, persist: false, reason: 'restore' });
  return {
    setCurrent,
    step,
    cancelPendingSelection,
    ...
    destroy() {
      destroyed = true;
      cancelPendingSelection();
      root.classList.remove('album-carousel', 'album-carousel-dragging', 'album-carousel-no-animation');
      root.style.removeProperty('--album-carousel-drag-x');
