/* GENERATED_FROM=input.js SOURCE_SHA256=a24eece76b3efecf512c0fc27e311a6f7b9693f24fa98dc2b1b812ab2a7ea152 FORMAT=READABLE_COMPACT PRINT_WIDTH=320 BLANK_LINES=SAFE_REMOVE DO_NOT_EDIT */
  ...
  async check({ force = false, reason = '' } = {}) {
    ...
    try {
      const statsArr = await metaDB.getAllStats(),
        gStat = statsArr.find(s => s.uid === 'global')?.featuresUsed || {};
      const trStats = statsArr.filter(s => s.uid !== 'global'),
        strk = (await metaDB.getGlobal('global_streak'))?.value?.current || 0;
      const favCount = window.FavoritesManager ? window.FavoritesManager.getSnapshot().filter(i => !i.inactiveAt && !i.deletedAt).length : 0;
      const agg = trStats.reduce(
        (a, b) => {
        ...
        },
        {
          ...
          favCount,
          streak: strk,
          play11_11: gStat.play_11_11 || 0,
          weekendPlays: gStat.weekend_play || 0,
          backups: gStat.backup || 0,
          pwaInstalled: gStat.pwa_installed || 0,
          sleepTimerTriggers: gStat.sleep_timer || 0
        }
      );
      if (window.TrackRegistry) {
        const allReg = window.TrackRegistry.getAllUids().map(u => window.TrackRegistry.getTrackByUid(u)),
          playedUids = new Set(trStats.filter(s => s.globalFullListenCount > 0).map(s => s.uid));
      ...
      for (const [k, r] of Object.entries(this.dict)) {
        ...
        if (r.type === 'static' && !this.unlocked[k]) {
          if (r.trigger.conditions.every(c => this._evalCondition({ ...c, target: c.target }, agg))) {
            unlock(k, r.ui.name, r.ui.icon);
            chg = true;
          }
          ...
          while (safety-- > 0 && this._hasScalableLevel(r, lvl)) {
            const completed = r.trigger.conditions.every(condition => this._evalCondition({ ...condition, target: this._getSc(r, lvl, false) }, agg));
            if (!completed) break;
            const id = `${k}_${lvl}`;
            unlock(id, r.ui.name.replace('{level}', lvl), r.ui.icon);
            chg = true;
            lvl++;
          }
        }
      ...
      this.lastAgg = agg;
      if (chg) {
        await Promise.all([metaDB.setGlobal('unlocked_achievements', this.unlocked), metaDB.setGlobal('achievement_unlock_meta', this.unlockMeta)]);
        window.ListeningReceipts?.refreshStatus?.().catch(() => null);
        try {
          window.dispatchEvent(new CustomEvent('backup:domain-dirty', { detail: { domain: 'achievements', immediate: true } }));
        } catch {}
      }
      this.achievements = this._buildUIArray();
