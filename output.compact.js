/* GENERATED_FROM=input.js SOURCE_SHA256=6d6c6d4a04adc803afa69c3c109c00c08a22c1763a0253c2b9368bf311a122c3 FORMAT=READABLE_COMPACT PRINT_WIDTH=320 BLANK_LINES=SAFE_REMOVE DO_NOT_EDIT */
...
class FavoriteMirrorService {
  ...
  startPolling() {
    this.stopPolling();
    if (document.hidden || !this.isAuthorized()) return;
    this.timer = setInterval(() => {
      if (document.hidden || !this.isAuthorized() || !networkAllowed()) return;
      this.sync().catch(() => null);
    }, POLL_MS);
  }
  stopPolling() {
