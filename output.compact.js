/* GENERATED_FROM=input.js SOURCE_SHA256=d2725452a4ca49dd30c50c85acb10f0894deaf8f6c9398a6009c2e1056e8d217 FORMAT=READABLE_COMPACT PRINT_WIDTH=320 BLANK_LINES=SAFE_REMOVE DO_NOT_EDIT */
...
const safeRangeId = value =>
  safe(value)
    .replace(/[^A-Za-z0-9._-]/g, '')
    .slice(0, 160);
const buildRangeKey = ({ deviceId = '', chainId = '', branchId = '', fromSeq = 0, toSeq = 0, hash = '' } = {}) => `${safeRangeId(deviceId)}:${safeRangeId(chainId || branchId)}:${Math.max(0, Math.floor(Number(fromSeq) || 0))}:${Math.max(0, Math.floor(Number(toSeq) || 0))}:${safeRangeId(hash).slice(0, 64)}`;
...
const countMap = raw =>
  Object.fromEntries(
    Object.entries(raw && typeof raw === 'object' && !Array.isArray(raw) ? raw : {})
      .map(([key, value]) => [safe(key), num(value)])
...
const sha256Hex = async value => [...new Uint8Array(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(JSON.stringify(sortObject(value)))))].map(byte => byte.toString(16).padStart(2, '0')).join('');
...
export const emptyStatsProjection = () => ({
  ...
  globalFeatures: {},
  v4: emptyStatsV4(),
  dimensions: { quality: {}, variant: {}, platform: {}, deviceClass: {}, deviceOs: {}, deviceBrowser: {}, pwa: {}, shuffle: {}, repeat: {}, favoritesOnly: {}, launchSource: {} }
});
const ensureTrack = (projection, uid) =>
  (projection.tracks[uid] ||= {
    listenMs: 0,
    uniqueCoveredMs: 0,
    completionBasisPointsSum: 0,
    analysisEligibleSessions: 0,
...
export const normalizeStatsProjection = raw => ({
  ...
  tracks: Object.fromEntries(
    Object.entries(raw?.tracks && typeof raw.tracks === 'object' ? raw.tracks : {})
      .map(([uid, row]) => [
        safe(uid),
        {
...
export const mergeProjectedStatsRow = (leftRaw = {}, rightRaw = {}) => {
  ...
  if (!uid) return null;
  if (uid === 'global') {
    const leftV4 = leftRaw.statsV4 || { cube: leftRaw.sparseCube || {}, repeat: leftRaw.repeatRuns || {}, boundary: { chainKey: '', firstRun: null, lastRun: null, singleRun: false } };
    const rightV4 = rightRaw.statsV4 || { cube: rightRaw.sparseCube || {}, repeat: rightRaw.repeatRuns || {}, boundary: { chainKey: '', firstRun: null, lastRun: null, singleRun: false } };
    const statsV4 = mergeStatsV4(leftV4, rightV4);
    const dimensions = {};
    const names = new Set([...Object.keys(leftRaw.dimensions || {}), ...Object.keys(rightRaw.dimensions || {})]);
    names.forEach(name => {
  ...
  const completionSum = num(leftRaw.averageCompletionRate) * leftSessions + num(rightRaw.averageCompletionRate) * rightSessions;
...
export default { STATS_SHARD_VERSION, emptyStatsProjection, buildStatsProjection, normalizeStatsProjection, buildStatsProjectionShard, verifyStatsProjectionShard, mergeStatsProjectionInto, mergeStatsProjections, projectionToStatsRows, mergeProjectedStatsRow, projectionStreak };
